...
import { Edit, Trash2, Film, ShoppingBag, Plus } from 'lucide-react';
import { AlertDialogTrigger } from '@/components/ui/alert-dialog';

const AdminContentList = ({ activeTab, homeInfo, galleryItems, products, onEdit, onDeleteInitiate, onAddNew }) => {
    ...
    return (
      <Card className="bg-black/40 border-red-900/30">
        ...
        <CardContent className="p-6 space-y-3">
          ...
          <p className="text-sm"><strong className="text-yellow-400">Twitter/X:</strong> <span className="text-white">{homeInfo.twitter_url || "Não definido"}</span></p>
  ...
  const itemsToDisplay = activeTab === 'gallery' ? galleryItems : products;
  const itemTypeLabel = activeTab === 'gallery' ? 'Item da Galeria' : 'Produto';
  const ItemIcon = activeTab === 'gallery' ? Film : ShoppingBag;

  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-white flex items-center">
          <ItemIcon className="h-6 w-6 mr-3 text-yellow-400" /> Gerenciar {activeTab === 'gallery' ? 'Galeria' : 'Produtos'}
        </h2>
        <Button className="bg-yellow-500 hover:bg-yellow-600 text-black" onClick={() => onAddNew(activeTab)}>
          <Plus className="h-4 w-4 mr-2" /> Adicionar {itemTypeLabel}
        </Button>
      </div>
      {itemsToDisplay.length === 0 && (
        <Card className="bg-black/40 border-dashed border-red-900/50">
          <CardContent className="p-10 text-center">
            ...
            <h3 className="text-xl font-semibold text-gray-400">Nenhum {itemTypeLabel.toLowerCase()} encontrado.</h3>
            <p className="text-gray-500">Clique em "Adicionar {itemTypeLabel}" para começar.</p>
          </CardContent>
        </Card>
      )}
            ...
            <CardContent className="p-6">
              <div className="flex justify-between items-start">
                ...
                <div className="flex flex-col space-y-2 ml-4">
                  <Button variant="outline" size="icon" onClick={() => onEdit(activeTab, item)} className="border-yellow-500 text-yellow-500 hover:bg-yellow-500 hover:text-black">
                    <Edit className="h-4 w-4" />
                  </Button>
                  <AlertDialogTrigger asChild>
                    <Button variant="outline" size="icon" onClick={() => onDeleteInitiate(activeTab, item.id)} className="border-red-500 text-red-500 hover:bg-red-500 hover:text-white">
